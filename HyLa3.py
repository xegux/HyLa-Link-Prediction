...
def decode(h, idx):
    h1, h2 = torch.index_select(h, 0, idx[0]), torch.index_select(h, 0, idx[1])
    return torch.sum(h1 * h2, dim=1)

def compute_metrics(HyLa_features, data, split):
    if split == 'train':
        edges_false = data[f'{split}_edges_false'][np.random.randint(0, len(data['train_edges_false']), len(data['train_edges']))]
    else:
        edges_false = data[f'{split}_edges_false']
    pos_scores = decode(HyLa_features, data[f'{split}_edges'])
    neg_scores = decode(HyLa_features, edges_false)
...
def train(model_f,
    ...
    for epoch in range(opt.epoch_start, opt.epochs):
        ...
        HyLa_features = torch.mm(
            data['features'].to(opt.device), HyLa_features)
        #del HyLa_features
        train_metrics = compute_metrics(HyLa_features, data, 'train')
        train_metrics['loss'].backward()
        optimizer_f.step()
        optimizer_c.step()
        optim_b.step()
        train_roc = train_metrics['roc']
        val_roc = compute_metrics(HyLa_features, data, 'val')['roc']
...
def main():
    ...
    parser.add_argument('-task', type=str, default='lp', help='learning task')
    ...
    opt.metric = 'roc' if opt.task == 'lp' else opt.metric
    opt.epoch_start = 0
    ...
    if opt.use_feats or opt.inductive:
        ...
        print(type(data['adj_train']))
        input()
        feature_dim = data['adj_train'].size(1)
    
    if opt.progress:
        # log.info(
        #     f'info about the data, training set size : {data[''].size(0)}')
        log.info(
    ...
    model_c = get_model("GraphSAGE", opt.hyla_dim, opt.he_dim).to(opt.device)
    ...
    test_roc = compute_metrics(model_f, data, 'test')['roc']
