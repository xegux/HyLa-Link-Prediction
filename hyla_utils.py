...
def mask_edges(adj, val_prop, test_prop, seed):
    ...
    adj_train = sp.csr_matrix((np.ones(
        train_edges.shape[0]), (train_edges[:, 0], train_edges[:, 1])), shape=adj.shape)
...
def load_data_lp(dataset, use_feats, data_path):
    if dataset in ['cora', 'pubmed']:
        ...
        adj, features = load_data_airport(
            dataset, data_path, return_label=False)
    else:
...
def load_data_nc(dataset, use_feats, data_path, split_seed):
    if dataset in ['cora', 'pubmed', 'citeseer']:
        ...
        if dataset == 'disease_nc':
            ...
            adj, features, labels = load_data_airport(
                dataset, data_path, return_label=True)
            val_prop, test_prop = 0.15, 0.15
        else:
            raise FileNotFoundError(
                'Dataset {} is not supported.'.format(dataset))
        idx_val, idx_test, idx_train = split_data(
            labels, val_prop, test_prop, seed=split_seed)
...
def load_reddit_data(data_path):
    ...
    data = {'adj_all': adj, 'adj_train': train_adj, 'features': features, 'labels': labels,
            'idx_train': train_index, 'idx_val': val_index, 'idx_test': test_index}
...
def load_citation_data(dataset_str, use_feats, data_path, split_seed=None):
    ...
    test_idx_reorder = parse_index_file(os.path.join(
        data_path, "ind.{}.test.index".format(dataset_str)))
...
def load_synthetic_data(dataset_str, use_feats, data_path):
    ...
    for i, j in edges:
        ...
        adj[j, i] = 1.
    if use_feats:
        features = sp.load_npz(os.path.join(
            data_path, "{}.feats.npz".format(dataset_str)))
    else:
        features = sp.eye(adj.shape[0])
    labels = np.load(os.path.join(
        data_path, "{}.labels.npy".format(dataset_str)))
...
def load_ppi(data_path):
    ...
    feat_sub = np.empty(
        (len(nodes_per_graph), subgraph_nodes, features.shape[1]))
...
def load_corpus(data_dir, dataset_str, inductive=False):
    ...
    for p in phases:
        index_dict[p] = load_pkl(
            "{}/ind.{}.{}.x".format(data_dir, dataset_str, p))
        label_dict[p] = load_pkl(
            "{}/ind.{}.{}.y".format(data_dir, dataset_str, p))
...
#         nonzero_perc.append("%.2f" % percentage)
    ...
    for phase in ["test", "val"]:
        ...
        # adj is symmetric!
        feat_dict[phase] = ((feats-train_feats_min) /
                            train_feats_range).cpu().double()
...
def sgc_precompute_text_v1(adj, features, degree, index_dict):
    ...
    for phase in ["test", "val"]:
        ...
        feats = feats[:, useful_features_dim]
        feat_dict[phase] = ((feats-train_feats_min) /
                            train_feats_range).cpu()  # adj is symmetric!
    precompute_time = perf_counter()-start
    return feat_dict, precompute_time
